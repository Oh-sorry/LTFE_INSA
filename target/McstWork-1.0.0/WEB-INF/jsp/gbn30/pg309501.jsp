...
$(document).ready(function() {
	$("#grid").jqGrid({
	   	url:"<c:url value='/gbn30/pg309501List.ajax' />",
	   	mtype: "POST",
		datatype: "json",
		locale : "kr",
		//postData : {'code':'00000'},
		...
		colModel:[
			{label:'구분코드', name:'menuGubn1', align:'center'},
			{label:'프로그램(구분)명', name:'menuNm', align:'center'}
	   	],
	   	loadonce: true,
	   	sortable : true,
	   	showpage : false,
...
function goInput() {
	...
	$('#inputForm').attr("action", "<c:url value='/gbn30/pg309501Input.do'/>");
...
function goModify() {
	...
	window.open('about:blank','input','width=600px,height=600px,top=0,left=0,toolbar=no,status=no,menubar=no,location=no,scrollbars=yes')
	$('#inputForm').attr("action", "<c:url value='/gbn30/pg309501Modify.do'/>");
	$('#inputForm').attr("target", "input");
	inputForm.submit();
}
...
function goDelete() {
	...
	$.ajax({
         type : "POST",
         url : "<c:url value='/gbn30/pg309501Delete.ajax' />",
	...
	<div id="wrap">
        ...
        <div id="content_wrap">
			...
			<form name="inputForm" id="inputForm" method="post">
				<input type="hidden" name="menuGubn1" id="menuGubn1">
